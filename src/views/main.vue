...
export default {
  ...
  computed: {
    ...
    orgNo: {
      get () {
        return this.$store.state.user.orgNo
      },
      set (val) {
        this.$store.commit('user/updateOrgNo', val)
      }
    },
    orgName: {
      get () {
    ...
    getUserInfo () {
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.orgNo = data.user.orgNo
          this.$cookie.set('drugStoreId', data.user.orgNo)
          // this.getOrgName(this.orgNo)
        }
      })
    },
    // 获取name
