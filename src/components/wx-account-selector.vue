<template>
  <el-select style="width: 240px" v-model="selectedAppid" size="small" v-loading="dataListLoading"
             @change="selectAccount" filterable>
    <el-option v-for="item in accountList" :key="item.appid" :label="item.name+'（'+ACCOUNT_TYPES[item.type]+'）'"
